...
const ExpenseList: React.FC<ExpenseListProps> = ({ expenses, members, onDeleteExpense, onEditExpense, onViewExpense, hasActiveFilters, originalExpenseCount, currentUserId }) => {
  if (originalExpenseCount === 0) {
    return (
      <div className="text-center py-16 px-6 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
        ...
        <h3 className="mt-3 text-base font-medium text-text-primary-light dark:text-text-primary-dark">No expenses recorded</h3>
  ...
  if (expenses.length === 0 && hasActiveFilters) {
     return (
      <div className="text-center py-12 px-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
        <FilterIcon className="mx-auto h-10 w-10 text-gray-400" />
        <h3 className="mt-3 text-base font-medium text-text-primary-light dark:text-text-primary-dark">No Matching Expenses</h3>
